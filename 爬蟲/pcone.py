...
starturl = 'https://www.pcone.com.tw/product/199'
#起始頁
...
#取得頁面所有的超連結
allurl = 'ul.static-products.product-list a'
...
for allprod in range(0,urllen0):
    ...
    driver.get('https://www.pcone.com.tw'+allurl0[allprod].get('href'))  # 輸入超連結，交給瀏覽器 
    pageSource = driver.page_source  # 取得網頁原始碼
    soup = BeautifulSoup(pageSource,"lxml") #將資料用lxml裝起來放置到soup
    
    
    #以下是取出需要的資料------------------以下是第二階段
    main_images = 'div.swiper-slide.trigger-zoom'
    #圖片
    main_name = 'h1.product-name'
    #標題 
    original_price = 'span.original'
    #原價
    special_price = 'span.discount'
    #價格
    file = 'div.detail-row.padding-block.images.description'
    #主介紹
    ...
    try:
        ...
        for imgnum in range(imglen):
            #main_images0[imgnum].re.findall(url)
           # main_images1.append(main_images0[imgnum][background-image:])
            main_images1 = main_images0[imgnum].re.search("url\")#----------------------------圖片有問題
    except IndexError:
        main_images1 = ''
        #print(main_images1)
    ...
    try:
        ...
        file1 = ''
        #print(file1)
    nowurl = 'https://www.pcone.com.tw'+allurl0[allprod].get('href')

    try:
        # success
        cursor.execute('INSERT INTO '+ ' pcone (main_images, main_name, original_price, special_price, file, time, url)' + ' VALUES(%s,%s,%s,%s,%s,NOW(),%s)' ,
